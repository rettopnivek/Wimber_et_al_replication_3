...
for trl = st_trials:end_trials
    ...
    % Extract label for image
    labelString = char( Labels{1}( all_labels( pairings( image_type, ind(trl) ) ) ) );
    ...
    stimulusInfo{incStimInfo} = [ num2str(curTrial) ',' num2str(expCond) ...
        ',' num2str(blck) ',' f_name ',' labelString ',' cueString ];
