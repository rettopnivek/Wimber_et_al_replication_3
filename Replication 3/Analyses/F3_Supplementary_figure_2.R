...
if ( savePlot ) {
  setwd( 'Figures' )
  if ( panelYes ) pdf( 'Supplementary_figure_2.pdf' ) else 
    pdf( 'Supplementary_figure_2_no_label.pdf' )
  setwd( orig_dir )
} else x11()
...
axis( 2, c( 0, .25, .5, .75, 1 ),
      c( '0', '25', '50', '75', '100' ),
      cex.axis = txtSz*.8, tick = F, line = -.5 )
...
axis( 1, 1:2, c('Baseline', 'Selective retrieval'),
      tick = F, cex.axis = txtSz*.8, line = -.5 )
axis( 2, c( .7, .8, .9 ),
      c( '70', '80', '90' ),
      tick = F, line = -.5, cex.axis = txtSz*.8 )
...
# TB CB TSR CSR
# CB TB TSR CSR
xao = c( 1.2, 0.8, 1.8, 2.2 )
xar = c( 1.2, 0.8, 1.8, 2.2 )
...
segments( xao[1:2], est_frm_od$frm$x[1:2],
          xao[3:4], est_frm_od$frm$x[3:4],
          col = 'red', lty = 1:2, lwd = lnSz )
points( xao, est_frm_od$frm$x, pch = c( 21, 22, 21, 22 ),
        col = 'red', bg = c('red','white','red','white'),
        cex = ptSz )
points( xar, est_frm_rd$frm$x, pch = c( 21, 22, 21, 22 ),
...
segments( xar[1:2], est_frm_rd$frm$x[1:2],
          xar[3:4], est_frm_rd$frm$x[3:4],
          col = 'black', lty = 1:2, lwd = lnSz )
points( xao, est_frm_od$frm$x, pch = c( 21, 22, 21, 22 ),
        col = 'red', bg = c('red','white','red','white'),
        cex = ptSz )
points( xar, est_frm_rd$frm$x, pch = c( 21, 22, 21, 22 ),
