...
if ( savePlot ) {
  setwd( 'Figures' )
  if ( panelYes ) pdf( 'SA_results.pdf' ) else 
    pdf( 'SA_results_no_label.pdf' )
  setwd( orig_dir )
} else x11()
...
legend( 'topright', c( 'Original', 'Low intrusions', 
                      'High visualization', 'Replication' ),
...
for ( i in 1:2 ) {
  ...
  if ( i == 1 ) {
    ...
    axis( 2, seq( .7, 1, .1 ),
          paste( 100*seq( .7, 1, .1 ), '%', sep = '' ),
          tick = F, cex.axis = txtSz*.9 )
