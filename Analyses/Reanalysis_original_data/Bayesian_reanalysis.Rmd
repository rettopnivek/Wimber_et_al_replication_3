...
# We'll use Stan to fit a generalized linear model
library( rstanarm )
...
```{r}
sel = OriginalAllData$Cond == 2 | OriginalAllData$Cond == 3 | OriginalAllData$Cond == 4
...
tmp = aggregate( dSR$Response==Targets, list( imgTar, dSR$Subject ), 
                 mean  )
tmp2 = aggregate( dSR$Response==Competitors, list( imgCom, dSR$Subject ), 
                 mean  )
...
fit = stan_glmer(cbind(Y, Trial) ~ RIF + (1|S) + (1|I), data = d, family = binomial("logit"), prior_intercept = normal(1.775,.3), prior = normal( -.3, .3 ), prior_PD = TRUE,chains = 4, cores = 4, seed = 7453)
...
With these priors, the model predicts an average accuracy of 80% across subjects, and a RIF effect of about 4%. Accuracy ranges roughly between 60 to 90% though. We'll now fit the model to the actual data and update our priors.

```{r}
# Fit the model to the original data
fit = stan_glmer(cbind(Y, Trial) ~ RIF + PT + (1|S) + (1|I), data = d, family = binomial("logit"), prior_intercept = normal(1.775,.3), prior = normal( c(-.3,0), c(.3,.3) ), chains = 8, cores = 8, seed = 3872,iter=1250,warmup=500)
...
```{r,echo=FALSE}
violinPlot = function( x, pos, crit = NULL, scaleH = .5, type = 'greater', ... ) {
  den = density( x )
...
plot( c(0,1), c(-.2,.8), type = 'n',
      xaxt = 'n',  xlab = 'Previous practice', ylab = ' ', bty = 'n' )
abline( h = 0, lty = 2 )
postProb = violinPlot( post[,3], .5, scaleH = .4, crit = 0, 
                       type = "greater", border = NA, col = 'grey' )
violinPlot( post[,3], .5, scaleH = .4 )
legend( 'topleft', paste( 'P( Theta > 0 ) = ', round(postProb,2) ),
        bty = 'n' )
...
nd = d[,c('Y','I','S','IT','SR','RIF','PT','Trial')]; nd$Y = 0;
Sim = posterior_predict( fit, newdata = nd )
...
The model does quite well at the group-level, but (unsurprisingly) is less precise at the subject level. Finally, assuming that the posteriors are well approximated by normal distributions, we can use the mean and standard deviation of the posterior approximations for the intercept and RIF coefficient to determine the subsequent priors for the replication analyses.
