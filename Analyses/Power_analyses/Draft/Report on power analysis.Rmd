  ...
  for ( i in 1:length( RIF_effect ) ) {
    ...
    sim_function = function() {
      ...
      RIF_cond = as.vector( apply( matrix(NA,Ss,Is), 1, 
                                   function(x) x = sample( c( rep(0,18+18+54), rep(1,54) ) ) ) )
      # Design matrix
      X = cbind( 1, RIF_cond )
    ...
    startTime = Sys.time()
    sim_results = apply( cbind( p_value ), 1, function(x) x = sim_function() )
...
load('Power_100_rep.RData') # Load in previously calculated values

plot( 1:ncol(power), power[1,], ylim = c(0,1),
      bty = 'l', xaxt = 'n', xlab = 'RIF effect',
      ylab = 'Power', type = 'n' )
axis( 1, 1:5, paste( 100*round( logit(1.7) - logit( 1.7 + RIF_effect ), 2 ),
                  '%',sep=''), tick = F )
abline( h = .05 )
abline( h = .8, lty = 2 )
abline( h = .5, lty = 3 )
...
Note that the predicted power can vary between approximately +/- 5% since I only use 100 repetitions.