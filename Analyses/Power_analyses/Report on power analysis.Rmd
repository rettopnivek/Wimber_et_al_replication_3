...
load('SuccRep_1000_rep.RData') # Load in previously calculated values

plot( 1:ncol(repMat), repMat[1,], ylim = c(0,1), 
			type='n', xaxt = 'n', bty = 'l', 
			xlab = 'RIF effect', ylab = 'Power' )
axis( 1, 1:5, paste( 100*round( logit(1.74) - logit( 1.74 + RIF_effect ), 2 ),
                  '%',sep=''), tick = F )
...
pts = c( 8, 17, 19 )
for (j in 1:nrow(repMat)) {
  lines( 1:ncol(repMat), repMat[j,] )
  points( 1:ncol(repMat), repMat[j,], pch = pts[j] )
}
